...
All examples are only deployed on our local host, which means all microservices are only running on local Docker. All access to microservices are through <text> http://localhost:<port>/<controller name>/... </text> or http://<microservices name>:<port>/<controller name>/... Thus, some issues are remained here and can be easily solved when using domain or public IP.
